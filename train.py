...
#add new tokens from new dataset to gpt2 model    
    # array = [['a','b'], ['a', 'b','c'], ['a']]
    unique=()
    for item in gold_train:
        for entry in item:
            if item.index(entry) != 0:
                unique = list(set(unique) | set(entry.split()))

    
#add these tokens to the dictionary otherwise ex. model considers [ENT] as 
